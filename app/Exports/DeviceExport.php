    ...
    public function collection()
    {
        ...
        foreach ($dvs as $row) {
            ...
            $dv[] = array(
                ...
                '11' => "Thiết bị chưa bàn giao sử dụng",
                '12' => $row->price,
                '13' => $row->import_date,
                '14' => \App\Provider::where(['id'=>$row->provider_id])->pluck('provider_name')->first(),
                '15' => $row->handover_date,
            ...
            elseif ($row->status == 1){
                ...
                '9' => $row->country,
                '10' => \App\Department::where(['id'=>$row->department_id])->pluck('department_name')->first(),
                '11' => "Thiết bị đang được sửa chữa",
                '12' => $row->price,
                '13' => $row->import_date,
                '14' => \App\Provider::where(['id'=>$row->provider_id])->pluck('provider_name')->first(),
                '15' => $row->handover_date,
